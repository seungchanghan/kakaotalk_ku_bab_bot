...
챗봇의 스킬 요청에 응답하는 무료 운영용 프로젝트입니다.
...
고려대 공식 식단 페이지
  ...
  → GitHub Pages: public/data/menu.json
  → Cloudflare Worker: 카카오 Skill JSON 변환
  → 카카오톡 채널 챗봇
```
...
수집이 전부 실패하면 배포 단계로 넘어가지 않아 마지막 정상 배포가 유지됩니다.
...
식당을 지정하면 상세 메뉴를, 지정하지 않으면 식당별 첫 메뉴를 요약합니다.
